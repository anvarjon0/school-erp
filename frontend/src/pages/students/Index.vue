  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div class="flex items-center space-x-3">
        <button @click="openCreateForm" class="inline-flex items-center justify-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead class="bg-gray-50/50 dark:bg-gray-800/50">
            <tr>
              ...
              <th scope="col" class="relative px-6 py-4 w-40"></th>
            </tr>
          </thead>
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <tr v-for="student in students" :key="student.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-10 w-10">
                    <img v-if="student.photo" :src="`/storage/${student.photo}`" class="h-10 w-10 rounded-full object-cover">
                    <div v-else class="h-10 w-10 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center text-primary-600 dark:text-primary-400 font-bold">
                  ...
                  <div class="ml-4">
                    ...
                    <div class="text-xs text-gray-500 dark:text-gray-400">
                      {{ student.parent_info?.father_phone || student.parent_info?.mother_phone || 'Tel raqam yo\'q' }}
                    </div>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex items-center justify-end space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button title="To'lov qilish" class="p-1.5 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg transition-colors">
                    <Banknote class="w-4 h-4" />
                  </button>
                  <router-link :to="`/students/${student.id}`" title="Profilni ko'rish" class="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors">
                    <Eye class="w-4 h-4" />
                  </router-link>
                  <button @click="openEditForm(student.id)" title="Tahrirlash" class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors">
                    <Edit class="w-4 h-4" />
                  </button>
                </div>
              </td>
    ...
    <SlideOver 
      v-model="isFormOpen" 
      :title="editingStudentId ? 'O\'quvchini tahrirlash' : 'Yangi o\'quvchi qo\'shish'"
      width="max-w-md sm:max-w-lg"
    >
      <!-- We only mount the form when slideover is open so it resets state -->
      <StudentForm 
        v-if="isFormOpen"
