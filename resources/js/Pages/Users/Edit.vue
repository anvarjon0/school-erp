<template>
  <AdminLayout>
    <template #header>Xodimni tahrirlash: {{ user.name }}</template>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 max-w-3xl mx-auto overflow-hidden">
      <div class="p-6 border-b border-gray-100 flex justify-between items-center">
        <h3 class="text-lg font-bold text-gray-800">Ma'lumotlarni o'zgartirish</h3>
      ...
      <div class="p-6">
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100">
            <!-- Password -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Yangi Parol (ixtiyoriy)</label>
              <input v-model="form.password" type="password" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            ...
            <div>
              ...
              <input v-model="form.password_confirmation" type="password" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            </div>
          </div>
...
const form = useForm({
    ...
    role_id: props.user.roles && props.user.roles.length > 0 ? props.user.roles[0].id : '',
